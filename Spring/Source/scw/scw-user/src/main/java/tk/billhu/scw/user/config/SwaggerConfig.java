package tk.billhu.scw.user.config;
...
public class SwaggerConfig {
    ...
    public Docket adminApis() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("管理员模块").select()
                .apis(RequestHandlerSelectors.withClassAnnotation(Api.class)).paths(PathSelectors.regex("/admin.*"))
                .build().apiInfo(apiInfo()).enable(enable);
    }
