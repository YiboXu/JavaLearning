...
public class AdminRoleController {
    ...
    @ApiOperation(value="获取特定管理员的角色列表")
    @GetMapping("/getAssignedRole")
    public AppResponse<List<AdminRoleResponseVo>> getAssignedRole(@RequestParam(value="adminid") String adminid){
        List<AdminRoleResponseVo> adminRoleResponseVoList;

        try{
            adminRoleResponseVoList = tAdminRoleService.getAssignedRole(Integer.parseInt(adminid));
        }catch (Exception e){
            e.printStackTrace();
            log.error("获取特定管理员角色列表异常-{}",adminid);
            AppResponse response = AppResponse.fail(null);
            response.setMsg("获取特定管理员角色列表异常");
            return response;
        }

        return AppResponse.ok(adminRoleResponseVoList);
    }

    @ApiOperation(value="获取特定管理员可赋予的角色列表")
    @GetMapping("/getUnAssignedRole")
    public AppResponse<List<AdminRoleResponseVo>> getUnAssignedRole(@RequestParam(value="adminid") String adminid){
        List<AdminRoleResponseVo> adminRoleResponseVoList;

        try{
            adminRoleResponseVoList = tAdminRoleService.getUnAssignedRole(Integer.parseInt(adminid));
        }catch (Exception e){
            e.printStackTrace();
            log.error("获取特定管理员可赋予的角色列表-{}",adminid);
            AppResponse response = AppResponse.fail(null);
            response.setMsg("获取特定管理员可赋予的角色列表");
            return response;
        }
    ...
    AppResponse<String> assignAdminRoles(@RequestParam(value="adminId") String adminId, @RequestParam(value="roleIds") String roleIds){
        ...
        try{
            result = tAdminRoleService.assignAdminRoles(adminId,roleIds);
        }catch (Exception e){
            e.printStackTrace();
            log.error("赋予管理员角色失败-{}-{}",adminId,roleIds);
            AppResponse response = AppResponse.fail(null);
            response.setMsg("赋予管理员角色失败");
            return response;
        }
